...
eventBus.on('member:joined', ({ user_id, drawer_id }) => {
  notificationService.subscribeUserToDrawer(user_id, drawer_id);
  ...
  notificationService.sendAlert({
    ...
    routeData: { route_type: 1, route_id: drawer_id },
  });
});

eventBus.on('member:left', ({ user_id, drawer_id, actor_id, action }) => {
  notificationService.unsubscribeUserFromDrawer(user_id, drawer_id);
...
eventBus.on('device:registered', ({ user_id }) => {
  notificationService.subscribeUserToAllDrawers(user_id);
});
